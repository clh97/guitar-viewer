  ...
  const createFretFreq = () => {
    ...
    guitarNotes.forEach((gn, gnIndex) => {
      ...
      Array.from({ length: fretQuantity }, (cur, index) => {
        ...
        const frequencyNotation = (index) =>
          (notes[index] + frequencyFactor).toString();
        let frequency = getFrequency(frequencyNotation(tmpIndex));

        if (tmpIndex == notes.length - 1) {
          currentNote = notes[0];
          frequency = getFrequency(frequencyNotation(tmpIndex).toString());
          tmpIndex = 0;
          fret[gn].push({
            note: currentNote,
            freq: frequency,
  ...
  const updateIndicatorPosition = (e) => {
    ...
    if (yIndex == 0 || yIndex <= 5) {
      setIndicatorPosition({
        x:
          parseInt(xIndex * (totalWidth / fretQuantity)) +
          totalWidth / fretQuantity / 2,
        y: parseInt(
          height / (noteQuantity + 1) + yIndex * (height / (noteQuantity + 1))
        ),
      });
    }
  };
          ...
          {guitarNotes.map((note, index) => {
            return (
              <Text
                ...
                x="20"
                y={
                  height / (noteQuantity + 1) +
                  index * (height / (noteQuantity + 1)) +
                  8
                }
                textAnchor="middle"
              >
            ...
            return (
              <RNText
                ...
                style={{
                  ...
                  top:
                    height / (noteQuantity + 1) +
                    fretFreq[ffKey][nKey].position.posY -
                    8,
                }}
              >
